...
namespace Konnetic.Sip.Headers
    ...
    /// </example>
    /// </remarks> 
    public class SipParameterCollection : Collection<SipParameter>, IEnumerable<SipParameter>
        {
        #region Fields
        ...
        public IEnumerator<SipParameter> GetEnumerator()
            {
            return new SipParameterCollectionEnumerator(this);
            }
        ...
        public void CopyTo(Array array, int index)
            ...
            if(array.Length <= index)
                {
                throw new OverflowException("Index cannot be larger than array size");
                }
            if(base.Count > 0)
                {
                for(int i = 0; i < base.Count; i++)
        ...
        public SipParameterCollection(string seperator)
            : this()
            {
        ...
        public SipParameterCollection(SipParameter parameter)
            : this()
            {
        ...
        /// <exception cref="SipOutOfRangeException">Is raised when <paramref value="index"/> is out of the collection bounds.</exception>
        ...
        public void Insert(int index, string name, string value)
            ...
            //NULL is ValueLessParam
            PropertyVerifier.ThrowOnNullOrEmptyString(name, "name");
            PropertyVerifier.ThrowIfIntOutOfRange(index, 0,Count,"index");
            if(this.Contains(name))
                {
        ...
        /// <threadsafety static="true" instance="false"/>
        public void Set(int index, SipParameter parameter)
            {
            PropertyVerifier.ThrowOnNullArgument(parameter, "parameter");
            if(Contains(parameter))
                {
